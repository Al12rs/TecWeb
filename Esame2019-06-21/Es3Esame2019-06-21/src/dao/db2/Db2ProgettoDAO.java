...
public class Db2ProgettoDAO implements ProgettoDAO {
	...
	static final String TABLE = "progetti";

	// Se relazione molti-a-uno aggiungi un id per la FK
	static final String ID = "idProgetto";
	static final String CODICE = "codiceProgetto";
	static final String NOME = "nomeProgetto";
	static final String ANNO = "annoInizio";
	...
	static final String insert = "INSERT " + "INTO " + TABLE + " ( " + ID + ", " + CODICE + ", " + NOME + ", " + ANNO
			+ ", " + DURATA + " " + ") " + "VALUES (?,?,?,?,?) ";
	...
	static String update = "UPDATE " + TABLE + " " + "SET " + CODICE + " = ?, " + NOME + " = ?, " + ANNO + " = ?, "
			+ DURATA + " = ? " + "WHERE " + ID + " = ? ";
	...
	static String create = "CREATE " + "TABLE " + TABLE + " ( " + ID + " INT NOT NULL PRIMARY KEY " + "," + CODICE
			+ " VARCHAR(50) NOT NULL UNIQUE, " + NOME + " VARCHAR(50), " + ANNO + " INT, " + DURATA + " INT " + ") ";
	...
	public void create(ProgettoDTO progetto) {
		...
		try {
			...
			PreparedStatement prep_stmt = conn.prepareStatement(Db2ProgettoDAO.insert);
			// --- b. Pulisci e imposta i parametri (se ve ne sono)
			prep_stmt.clearParameters();
			prep_stmt.setInt(1, progetto.getIdProgetto());
			prep_stmt.setString(2, progetto.getCodiceProgetto());
			prep_stmt.setString(3, progetto.getNomeProgetto());
			prep_stmt.setInt(4, progetto.getAnnoInizio());
			prep_stmt.setInt(5, progetto.getDurata());
	...
	public ProgettoDTO read(int id) {
		...
		try {
			// --- a. Crea (se senza parametri) o prepara (se con parametri) lo statement
			PreparedStatement prep_stmt = conn.prepareStatement(Db2ProgettoDAO.read_by_id);
			// --- b. Pulisci e imposta i parametri (se ve ne sono)
			prep_stmt.clearParameters();
			...
			// tupla
			if (rs.next()) {
				ProgettoDTO entry = new Db2ProgettoDTOProxy();
				entry.setIdProgetto(rs.getInt(ID));
				entry.setCodiceProgetto(rs.getString(CODICE));
				entry.setNomeProgetto(rs.getString(NOME));
				entry.setAnnoInizio(rs.getInt(ANNO));
				entry.setDurata(rs.getInt(DURATA));
				result = entry;
			}
			// --- e. Rilascia la struttura dati del risultato
			rs.close();
	...
	public boolean update(ProgettoDTO progetto) {
		...
		try {
			// --- a. Crea (se senza parametri) o prepara (se con parametri) lo statement
			PreparedStatement prep_stmt = conn.prepareStatement(Db2ProgettoDAO.update);
			// --- b. Pulisci e imposta i parametri (se ve ne sono)
			prep_stmt.clearParameters();

			prep_stmt.setString(1, progetto.getCodiceProgetto());
			prep_stmt.setString(2, progetto.getNomeProgetto());
			prep_stmt.setInt(3, progetto.getAnnoInizio());
			prep_stmt.setInt(4, progetto.getDurata());
			prep_stmt.setInt(5, progetto.getIdProgetto());
			// --- c. Esegui l'azione sul database ed estrai il risultato (se atteso)
			prep_stmt.executeUpdate();
			// --- d. Cicla sul risultato (se presente) pe accedere ai valori di ogni sua
			// tupla
	...
	public boolean delete(int id) {
		...
		try {
			// --- a. Crea (se senza parametri) o prepara (se con parametri) lo statement
			PreparedStatement prep_stmt = conn.prepareStatement(Db2ProgettoDAO.delete);
			// --- b. Pulisci e imposta i parametri (se ve ne sono)
			prep_stmt.clearParameters();
			prep_stmt.setInt(1, id);
			// --- c. Esegui l'azione sul database ed estrai il risultato (se atteso)
	...
	public Set<ProgettoDTO> readAll() {
		...
		try {
			// --- a. Crea (se senza parametri) o prepara (se con parametri) lo statement
			PreparedStatement prep_stmt = conn.prepareStatement(Db2ProgettoDAO.read_all);
			// --- b. Pulisci e imposta i parametri (se ve ne sono)
			prep_stmt.clearParameters();
			// --- c. Esegui l'azione sul database ed estrai il risultato (se atteso)
			ResultSet rs = prep_stmt.executeQuery();
