...
public class Db2PartnerDAO implements PartnerDAO {
	...
	static final String TABLE = "partners";

	// Se relazione molti-a-uno aggiungi un id per la FK
	static final String ID = "idPartner";
	...
	static final String insert = "INSERT " + "INTO " + TABLE + " ( " + ID + ", " + SIGLA + ", " + NOME + " " + ") "
			+ "VALUES (?,?,?) ";
	...
	// SELECT * FROM table WHERE idcolumn = ?;
	static String read_by_name = "SELECT * " + "FROM " + TABLE + " " + "WHERE " + SIGLA + " = ? ";
	...
	static String update = "UPDATE " + TABLE + " " + "SET " + SIGLA + " = ?, " + NOME + " = ? " + "WHERE " + ID
			+ " = ? ";
	...
	static String create = "CREATE " + "TABLE " + TABLE + " ( " + ID + " INT NOT NULL PRIMARY KEY " + "," + SIGLA
			+ " VARCHAR(50) NOT NULL UNIQUE, " + NOME + "VARCHAR(50) " + ") ";
	...
	public void create(PartnerDTO partner) {
		...
		try {
			...
			PreparedStatement prep_stmt = conn.prepareStatement(Db2PartnerDAO.insert);
			// --- b. Pulisci e imposta i parametri (se ve ne sono)
			prep_stmt.clearParameters();
			prep_stmt.setInt(1, partner.getIdPartner());
			prep_stmt.setString(2, partner.getSiglaPartner());
			prep_stmt.setString(3, partner.getNome());
	...
	public PartnerDTO read(int id) {
		...
		try {
			// --- a. Crea (se senza parametri) o prepara (se con parametri) lo statement
			PreparedStatement prep_stmt = conn.prepareStatement(Db2PartnerDAO.read_by_id);
			// --- b. Pulisci e imposta i parametri (se ve ne sono)
			prep_stmt.clearParameters();
			...
			if (rs.next()) {
				...
				entry.setIdPartner(rs.getInt(ID));
				entry.setSiglaPartner(rs.getString(SIGLA));
	...
	public boolean update(PartnerDTO partner) {
		...
		try {
			// --- a. Crea (se senza parametri) o prepara (se con parametri) lo statement
			PreparedStatement prep_stmt = conn.prepareStatement(Db2PartnerDAO.update);
			// --- b. Pulisci e imposta i parametri (se ve ne sono)
			prep_stmt.clearParameters();

			prep_stmt.setString(1, partner.getSiglaPartner());
			prep_stmt.setString(2, partner.getNome());
			prep_stmt.setInt(3, partner.getIdPartner());
			// --- c. Esegui l'azione sul database ed estrai il risultato (se atteso)
			prep_stmt.executeUpdate();
			// --- d. Cicla sul risultato (se presente) pe accedere ai valori di ogni sua
			// tupla
	...
	public boolean delete(int id) {
		...
		try {
			// --- a. Crea (se senza parametri) o prepara (se con parametri) lo statement
			PreparedStatement prep_stmt = conn.prepareStatement(Db2PartnerDAO.delete);
			// --- b. Pulisci e imposta i parametri (se ve ne sono)
			prep_stmt.clearParameters();
			prep_stmt.setInt(1, id);
			// --- c. Esegui l'azione sul database ed estrai il risultato (se atteso)
