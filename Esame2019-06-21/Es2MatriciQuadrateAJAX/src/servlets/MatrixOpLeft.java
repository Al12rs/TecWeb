...
public class MatrixOpLeft extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		Integer count = (Integer)session.getAttribute("countLeft");
		if(count == null) {
			session.setAttribute("countLeft", 1);
		} else if(count == 5) {
			resp.getWriter().println("Raggiunto il limite di operazioni");
		} else {
			count = count + 1;
			session.setAttribute("countLeft", count);
		}
