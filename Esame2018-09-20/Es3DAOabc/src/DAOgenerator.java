...
public class DAOgenerator
{
	...
	public static void main(String[] args) throws IOException
	{
		...
		objName="C"; //nome del bean (iniziale maiuscola!!)
		tableName="c"; //nome della tabella

		//non inserire "int id"!! (generato automaticamente)
		attr.add("int c1"); //coppie "tipo nome" separate da uno spazio
		attr.add("int c2");
		attr.add("int c3"); // foreign key
		///...A QUI
		...
		//GENERAZIONE DB2OggettoDAO
				...
				out.write("\tpublic "+objName+" read(int id);\n");
				out.write("\tpublic boolean update("+objName+" obj);\n");
				out.write("\tpublic boolean delete(int id);\n");
			...
			{
				String ret="";
				ret+="\"CREATE TABLE "+tableName+" (id INT NOT NULL PRIMARY KEY,";
				int index=0;
				for(String temp : attr)
				{
					...
					if(temp.split(" ").length==3)
					{
						...
						if(type.toUpperCase().equals("STRING"))
							ret+="VARCHAR(50)";
						else if(type.toUpperCase().equals("INT"))
							ret+="INT";
			...
			{
				String ret="";
				ret+="\t\t\tprep_stmt.setInt(1,obj.getId());\n";
				int index=2;
				for(String temp : attr)
				{
			...
			private static String generateUpdateCRUD(String word)
			{
				...
				ret+="\t\t\tprep_stmt.setInt("+index+",obj.getId());\n";
				System.out.println(ret);
				return ret;
			}
