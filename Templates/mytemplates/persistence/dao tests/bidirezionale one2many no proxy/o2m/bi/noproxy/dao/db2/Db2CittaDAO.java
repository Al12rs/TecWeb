...
import o2m.bi.noproxy.dao.CittaDAO;
...
public class Db2CittaDAO implements CittaDAO {
	...
	// incorrere in cicli infiniti si richiede al DAO di maratone il caricamento di 
	// maratone senza caricarne la città di appartenenza, città che viene però immediatamente
	// settata da questo metodo, visto che la sta creando lui
	@Override
	...
	public CittaDTO readByName(String name) {
		...
		try {
			...
			if (rs.next()) {
				...
				// lasciare che ogni maratona si ricostruisca da sola la città
				for (MaratonaDTO m : res.getMaratone()) {
					m.setCitta(res);
				}
