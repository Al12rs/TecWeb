	...
	public boolean update(VoloDTO obj) {
		...
		if (relationshipOwner) {
			...
			if (obj.getPasseggeri() == null || obj.getPasseggeri().isEmpty()) {
				...
				olds.stream().filter(oldId -> !news.contains(oldId)).forEach(oldId -> dao.delete(oldId, obj.getId()));
				news.stream().filter(newId -> !olds.contains(newId)).forEach(newId -> dao.create(newId, obj.getId()));
				// for(long oldId : olds) {
