var resultArray = [];

var responseHandlerTop = function (responseText, targetElement) {
	result = JSON.parse(responseText);
    if(!isNan(result)){
    	resultArray[0] = result;
    	if(resultArray[1] != -1){
    		var occ = resultArray[0] + resultArray[1];
    		targetElement.innerHTML = "Occorrenze: " + occ;
    	}
    }else{
    	targetElement.innerHTML = "Hai esaurito i tuoi conteggi."
    }
}

var responseHandlerBottom = function (responseText, targetElement) {
    result = JSON.parse(responseText);
    if(!isNan(result)){
    	resultArray[1] = result;
    	if(resultArray[0] != -1){
    		var occ = resultArray[0] + resultArray[1];
    		targetElement.innerHTML = "Occorrenze: " + occ;
    	}
    }else{
    	targetElement.innerHTML = "Hai esaurito i tuoi conteggi."
    }
}

function checkAndSendRequest(f) {

	resultArray[0] = -1;
	resultArray[1] = -1;
    var uri = "../splitcounter";
    var targetElement = myGetElementById("risultato");
    var postString = "filename="+f.fileName.value+"&option="+f.option.value;
    ajaxSendPOSTRequest(uri + "top", postString, targetElement, responseHandlerTop);
    ajaxSendPOSTRequest(uri + "bottom", postString, targetElement, responseHandlerBottom);

}


//template
function ajaxSendPOSTRequest(uri, postString, targetElement, responseHandler) {
    var xhr = myGetXmlHttpRequest();

    if (xhr) {
        // impostazione controllo e stato della richiesta
        xhr.onreadystatechange = function () { ajaxCallback(xhr, targetElement, responseHandler); };

        // impostazione richiesta asincrona in GET
        // del file specificato
        try {
            xhr.open("post", uri, true);
        }
        catch (e) {
            // Exceptions are raised when trying to access cross-domain URIs 
            alert(e);
        }

        // rimozione dell'header "connection" come "keep alive"
        xhr.setRequestHeader("connection", "close");

        // invio richiesta
        xhr.send(postString);
    }
    else
        targetElement.innerHTML = "Errore durante creazione xmlhttpRequest.";
}

//template
function ajaxCallback(xhr, targetElement, responseHandler) {
    // verifica dello stato
    if (xhr.readyState === 2) {
        //targetElement.innerHTML = "Richiesta inviata...";
    }// if 2
    else if (xhr.readyState === 3) {
        //targetElement.innerHTML = "Ricezione della risposta...";
    }// if 3
    else if (xhr.readyState === 4) {

        // verifica della risposta da parte del server
        if (xhr.status === 200) {

            // operazione avvenuta con successo
            responseHandler(xhr.responseText, targetElement);

        }// if 200

        else {
            // errore di caricamento
            targetElement.innerHTML = "Impossibile effettuare l'operazione richiesta.<br />";
            targetElement.innerHTML += "Errore riscontrato: " + xhr.statusText;
        }// else (if ! 200)

    }// if 4
}